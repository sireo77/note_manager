# Код по спецификации № 3.5 по Grade 1 :
# Префикс "a" обозначает локальную переменную, "t" - временную, промежуточную
# Функция выводит интерактивное текстовое меню, позволяющее пользователю выбирать действия.
#####################################   РУКОВОДСТВО ТЕСТИРОВЩИКА   #################################################
# Механизм работы вложенного меню: общий цикл, если ввод некорректен, то запрос о продолжении, иначе вход во вложенное
# меню. Если во вложенном меню ввод не корректен, то запрос о продолжении, иначе - вызов функционала
# Для работы со списком заметок нужно ввести '1' - Работа со списком заметок. В выпавшем меню будет доступно:
# 6. Выйти из программы - '0' - Выход;
# 1. Создать новую заметку - '1' - Режим добавления новой заметки;
# 2. Показать все заметки - '2' - Режим отображения всех заметок;
# 5. Найти заметки - '3' - Режим вывода отобранных заметок;
# 4. Удалить заметку - '4' - Режим удаления заметки;
# Поскольку обновить можно только отобранную заметку, то для доступа к этому нужно ввести '2' - Работа с конкретной заметкой,
# далее '1' - Поиск по порядковому номеру и '1' - Режим редактирования заметки;
# По коду:
# функция show_choose_user_actions - отображает кортеж форматированных надписей с номерами и возвращает выбранный номер
# в виде строки
#####################################   ОТКЛОНЕНИЯ ОТ СПЕЦИФИКАЦИИ   #################################################
# состав меню несколько отличен от требований: есть разделение на работу со списком заметок и с конкретной заметкой,
# поскольку сделана попытка структуризации меню
...
# Функция вывода конструктора меню
def show_choose_user_actions(t_menu_list):
    if not t_menu_list:
        print("Список заголовков меню пуст!")
        exit()
    print("Введите цифру для выбора элемента меню:")
    # enumerate одновременно крутит пару номер и значение в t_menu_list
    for index, item in enumerate(t_menu_list):
        print(f"{index} - {item}")
...
def create_note(t_lists, t_list, t_usual_use = True):  # t_lists для сохранения в списке всех заметок
    ...
    if t_usual_use:  # Обычное использование функции
...
def find_note(t_lists):  #
    ...
    while t_exit_allow == "1":  # Сторожевое условие
        ...
        elif t_act == "4":  # Поиск ключевому слову
            t_title = input("Введите ключевое слово, заметку с которым надо найти: > ")
            t_list, i = search_notes(t_lists, t_title, "Слово")
        elif t_act == "5":  # Поиск по статусу
            t_title = status_change()
...
def del_all_notes(t_lists):  #
    ...
    save_notes(t_lists)  # Заливка в файл
...
def sort_notes(t_lists):
    print("===== Сортировка заметок: =====")
    t_menu_list = ("Возврат в главное меню", "Заголовок", "Имя пользователя" , "Содержание заметки" , "Статус заметки" ,
               "Время создания" , "Срок выполнения" )  # Настроим кортеж отображаемых элементов

    # Выбор полей, по которым будет сортировка
    print("Выберите номер поля заметки, по которому нужно сортировать")
    t_act = show_choose_user_actions(t_menu_list)  # Выведем меню для выбора режима работы
    if t_act == '0':
        print("Возвращение в главное меню.")
        return
    t_sort_dir = str(input("Сортировать по возрастанию (нажмите ""1"") или по убыванию (любая другая)? > "))
    while t_act not in g_keys_of_notes:
        t_act = str(input("Выберите нужное число: > "))
        if t_act not in g_keys_of_notes:
            print(f"Должна быть цифра в диапазоне от 0 до {len(t_menu_list)}. Повторите ввод!")
    # Собственно сортировка
    # Параметр key в функции sorted указывает lambda-функцию, которая возвращает значение,
    # используемое для сортировки. Это значение извлекается из каждого элемента t_list итерируемого t_lists
    # по ключу словаря из строки t_act.
    # reverse = True - по убыванию
    t_lists_sort = sorted(t_lists,
                          key = lambda t_list: t_list[t_act],
                          reverse = bool(t_sort_dir != "1")  # если выбрана "1", то возрастание
                          )

    display_notes(t_lists_sort)  # Выводим результат на экран

    print("===== Сортировка заметок завершена =====")
    save_notes(t_lists_sort)  # Заливка в файл
    ...
    while a_ses_exit_allow == "1":
        ...
        if a_main_act == "0":  # Быстрый выход
            ...
            a_menu_list = ["Выход", "Режим добавления новой заметки", "Режим отображения всех заметок",
                           "Режим вывода отобранных заметок", "Режим удаления заметки",
                           "Режим сортировки заметок"]  # Настроим меню работы со списком заметок
            ...
            if a_act == "0":  # Быстрый выход
                ...
                a_note = create_note(a_notes, a_note)  # Вызов в обычном режиме с сохранением
...
# -= ТЕКСТ ПРОГРАММЫ =-
# Объявим глобальную переменную, живущую во всех функциях
g_keys_of_notes = ['1', '2', '3', '4', '5', '6']

print(Fore.GREEN + "=================================")
print("        ПРИВЕТСТВУЮ! \n"
      "Приступим к работе с заметками.")
print("=================================" + Fore.RESET)
