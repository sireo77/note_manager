...
def del_this_note(t_lists, t_element, t_elem_type):  #
    for i in range(0, len(t_lists)):  # Поиск по заданному ключу
        if t_elem_type == "Имя":
            ...
            if t_element == t_lists[i]["6"][len(t_lists[i]["6"])-1]:
                print(f'Элемент "{t_element}" присутствует в списке!')
                del t_lists[i]
                save_notes(t_lists)
                print("===== Заметка удалена =====")
                return
    print("К сожалению, указанные поисковые характеристики заявки не найдены. Попробуйте ещё раз!")
...
def del_note(t_lists):  #
    ...
    while t_exit_allow == "1":  # Сторожевое условие
        if len(t_lists) == 0:
            ...
            return
        t_menu_list = ["Выход", "Поиск для удаления по имени", "Поиск для удаления по текущему заголовку", "Удаление всех заметок"]
        ...
        if t_act == "0":  # Быстрый выход
            ...
            t_name = input("Введите текущий заголовок, заметку с которым надо удалить: > ")
            del_this_note(t_lists, t_name, "Заголовок")
...
while a_ses_exit_allow == "1":
    ...
    if a_main_act == "0":  # Быстрый выход
        ...
        print("Вы ввели некорректное число, режим не был активирован.")
        t_exit_allow = False
    a_ses_exit_allow = str(input("Продолжить работу с заметками (нажмите ""1"") или выйти (любая другая) > "))
